	...
	<view>
		...
		<view class="orders-item daifu" v-for="(oItem,oIndex) in order_list" :key="oIndex">
			...
			<view class="currency-block">
				...
				<p v-else class="currency-amount">商家赠送</p>
			</view>
			...
			<view class="button-block" v-if="oItem.gid == 3 && (oItem.status==4 || oItem.status==2 || oItem.status==1) && oItem.kw12 != 1">
				<view class="orders-button">
					
					<a class="go-pay color-white bg-hdsh" v-if="oItem.status==2" @click="clickOk(oItem,oIndex)">确认收货</a>
					
					<a class="cancel wl" v-if="oItem.status==5" @click.stop="wlClick(oIndex)">物流信息
			...
			<view class="button-block" v-if="oItem.gid == 1 && (oItem.status==4 || oItem.status==1 || oItem.status==9)">
				<view class="orders-button">
					<a class="cancel" @click="unSubscribe(oItem,oIndex)">退订</a>
				</view>
			</view>
			<view class="button-block" v-if="(oItem.gid == 2 || oItem.gid == 4 || oItem.gid == 8) && (oItem.status==4 || oItem.status==5 || oItem.status==1)">
				<view class="orders-button">
					<a class="cancel" @click="refund(oItem,oIndex)">退款</a>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			toDetail(x) {
				uni.navigateTo({
					url: '/pages/order/orderDetail/orderDetail?g=' + x.gid + '&id=' + x.id + '&status=' + x.status,
				});
			},
			/**
			 * 确认收货方法
